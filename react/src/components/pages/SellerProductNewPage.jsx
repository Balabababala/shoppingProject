...
function SellerProductNewPage() {
  ...
  const [mainImage, setMainImage] = useState(null); // 主圖
  const [extraImages, setExtraImages] = useState([]); // 其他圖
  ...
  useEffect(() => {
    fetch(`${API_BASE}/categories/leaf`)
      ...
      .then(data => {
        if (data) {
          // 假設 API 回傳陣列直接是 leaf category DTO
          setCategories(data);
        }
      })
